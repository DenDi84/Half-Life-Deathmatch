...
function draw.RoundedBoxTwoColored(radius, x, y, w, h, colTopLeft, colBottomRight, colCenter, thickness)
    thickness = thickness or 2
    draw.RoundedBox(radius, x, y, w, h, colBottomRight)
    draw.RoundedBox(radius, x, y, w - thickness, h - thickness, colTopLeft)
    local insideRadius = math.max(0, radius - thickness)
    draw.RoundedBox(insideRadius, x + thickness, y + thickness, w - (thickness * 2), h - (thickness * 2), colCenter)
end
...
function PANEL:Init()
    ...
    self.countDown:SetPos(self.Canvas:GetWide()/2, 14)
...
function PANEL:PerformLayout()
    ...
    self.Canvas:SetWide(extra-5)
    self.Canvas:SetTall(cy, -60)
    self.Canvas:SetPos(0, ScrH()/3)
    --self.Canvas:CenterHorizontal()
    self.Canvas:SetZPos(0)
    
    self.mapList:StretchToParent(0, 90, 0, 0)

    local buttonPos = 50 + extra - 31 * 3

    self.closeButton:SetPos(buttonPos - 31 * 0, 17)
...
function PANEL:AddVoter(voter)
    ...
    icon_container.Paint = function(s, w, h)
        draw.RoundedBoxTwoColored(4,0,0,w,h,Color(116, 128, 108),Color(36, 48, 28),Color(76, 88, 68),2)
    ...
    for k, v in RandomPairs(maps) do
        ...
        do
            ...
            button.Paint = function(s, w, h)
                ...
                draw.RoundedBoxTwoColored(4,0,0,w,h,Color(116, 128, 108),Color(36, 48, 28),col,1)
                Paint(s, w, h)
            end
        end
